...
Function MyCustomPage
  !insertmacro MUI_HEADER_TEXT "Custom Installation" "Installing..."

  ; Create progress bar
  StrCpy $ProgressBar 0
  ${NSD_CreateProgressBar} 0 40u 100% 10u
  Pop $ProgressBar
  SendMessage $ProgressBar ${PBM_SETRANGE} 0 100
  SendMessage $ProgressBar ${PBM_SETPOS} 0 0
  ;GetDlgItem $ProgressBar $ProgressBar ; (expect 3 parameter, but only 2)

  ; Create progress text label
  ${NSD_CreateLabel} 0 55u 100% 12u "Installing..."
  Pop $ProgressText

  ; Start the installation process in the background
  SetCurInstType manual ; Set the installation type to manual to prevent automatic page navigation
  SetDetailsPrint none  ; Suppress details output
  Call MyInstallationFunction
FunctionEnd

; Installation function (replace this with your actual installation logic)
Function MyInstallationFunction
  StrCpy $ProgressBar 0
  StrCpy $ProgressText "Installing..."

  ; Perform your installation tasks here
  Sleep 500 ; Simulate installation progress

  StrCpy $ProgressBar 50
  StrCpy $ProgressText "50% Complete..."

  ; More installation tasks...
  Sleep 500 ; Simulate installation progress

  StrCpy $ProgressBar 100
  StrCpy $ProgressText "Installation Complete!"

FunctionEnd

; Callback function to show the custom page
Function .onInit
 ; !insertmacro MUI_INSTALLOPTIONS_EXTRACT ;(require 1 paramater, passed 0)

  ; Show the custom page
  nsDialogs::Create 1018
  Pop $0
  ;${NSD_CreatePage} 0 0 100% 100% ;(Invalid command)
  Pop $0
  ;${NSD_AddEx} $0 0 0 100% 100% "" "MyCustomPage" ;(Invalid command)
  nsDialogs::Show
FunctionEnd
;-------------------------------------------
